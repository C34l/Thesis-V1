...
x_1_alpha = h.np.array([0.01650, 0.01900, 0.01600, 0.01800, 0.01750, 0.01750, 0.01800, 0.01700, 0.01700, 0.01750, 0.01750, 0.01600, 0.01700, 0.01600, 0.01850, 0.01250, 0.01850, 0.01850, 0.02250, 0.02250, 0.02250, 0.02450, 0.02500, 0.02850, 0.03100, 0.03450, 0.03700, 0.04100, 0.04650, 0.05300, 0.05700, 0.07750, 0.08450, 0.09700, 0.10200])
t_exp = h.np.array([290.55649, 292.41147, 294.94098, 296.12142, 297.63912, 299.49410, 302.02361, 303.87858, 307.58853, 310.45531, 312.98482, 323.60877, 333.55818, 338.61720, 344.01349, 348.90388, 353.45700, 355.81788, 363.23777, 367.11636, 371.83811, 373.18718, 380.60708, 383.81113, 386.50927, 388.02698, 391.39966, 392.58010, 394.94098, 396.96459, 397.30185, 397.63912, 397.97639, 398.48229, 398.65093])
x_1_beta = h.np.array([0.4915, 0.4900, 0.4890, 0.4875, 0.4855, 0.4830, 0.4810, 0.4785, 0.4740, 0.4715, 0.4575, 0.4370, 0.4390, 0.4190, 0.4035, 0.3675, 0.3590, 0.3525, 0.3315, 0.3240, 0.2990, 0.2920, 0.2700, 0.2500, 0.2395, 0.2095, 0.1955, 0.1925, 0.1860, 0.1695, 0.1610, 0.1455, 0.1290, 0.1195, 0.1105])
y = h.np.array([291.1, 293.4, 296.5, 297.8, 299.8, 302.0, 305.2, 308.4, 311.0, 313.7, 323.1, 331.9, 334.1, 343.5, 353.6, 360.5, 363.4, 365.8, 370.5, 372.3, 379.6, 379.9, 384.1, 387.9, 388.7, 393.4, 394.6, 395.8, 395.4, 397.3, 397.5, 397.5, 398.5, 398.7, 398.8, 398.7, 398.5])
...
for x in range(steps):
 x_output_beta[x] = h.spo.fsolve(phase_balance_for_x, x_1_beta[x], args=(x_1_alpha[x], t_exp[x]))
 print(x_1_beta[x])

x_output_alpha= h.spo.root(phase_balance_for_x, x_1_alpha, args=(x_1_beta, t_exp), method='lm')
#print('point_alpha:',x_output_alpha)

args_alpha = x_output_alpha.x

#print('array alpha:',args_alpha)

x_output_beta = h.spo.root(phase_balance_for_x, x_1_beta, args=(x_1_alpha, t_exp), method='lm')
#print('point_beta:',x_output_beta.x,'temp:',t_exp)

alpha_diff = h.np.zeros(steps)

for x in range(steps):
    alpha_diff[x] = x_1_alpha[x]-args_alpha[x]
    #print(alpha_diff[x])
    #print(args_alpha[x])
    #print(t_exp[x])
